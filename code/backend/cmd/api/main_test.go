...
func TestGreetingNotFound(t *testing.T) {
	...
	greeting(fakeGreetingDB{row: fakeRow{err: sql.ErrNoRows}}).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/greeting", nil))
...
func TestGreetingEmpty(t *testing.T) {
	rec := httptest.NewRecorder()
	greeting(fakeGreetingDB{row: fakeRow{err: nil}}).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/greeting", nil))
	if rec.Code != http.StatusUnprocessableEntity { t.Fatalf("status = %d", rec.Code) }
}

func TestGreetingInternalError(t *testing.T) {
	rec := httptest.NewRecorder()
	greeting(fakeGreetingDB{row: fakeRow{err: errors.New("db down")}}).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/greeting", nil))
